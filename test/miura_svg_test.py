...
def square_twist_test(do_n4b5=False):
    ...
    m = np.size(node, 0)
    supp = np.array([[5, 1, 1, 1], [4, 1, 1, 1], [8, 1, 1, 1], [14, 1, 1, 1], [22, 1, 1, 1], [17, 0, 0, 1], [6, 0, 0, 1]])
    load = np.array([[11, 0, 0, 50], [7, 0, 0, 50]], dtype=float)
    ...
    # visual_fold(u_his[:, :endicrm:interv], truss, angles, f_his[:endicrm:interv], instdof, intensity_map="vertex", intensity_data = v_intensity_data_inten, record_type = "imggif", filename="sqtwist_test")
...
cProfile.run("square_twist_test(False)", "stats.prof")
